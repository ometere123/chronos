...
import { Lock, LockOpen, CheckCircle2, XCircle, Clock, Shield, Plus, Banknote, Sparkles } from 'lucide-react';
...
export default function VaultCard({ vault }: VaultCardProps) {
  ...
  const statusIcons = {
    ACTIVE: <Lock className="w-3 h-3 inline mr-1 -mt-0.5" />,
    MATURE: <LockOpen className="w-3 h-3 inline mr-1 -mt-0.5" />,
    CLAIMED: <CheckCircle2 className="w-3 h-3 inline mr-1 -mt-0.5" />,
    FAILED: <XCircle className="w-3 h-3 inline mr-1 -mt-0.5" />,
  };

  const VaultTypeIcon = vault.vaultType === 'FIXED' ? Clock : Shield;
    ...
    <Link href={`/dashboard/${vault.vaultId}`}>
      <div className="card hover:border-primary/30 cursor-pointer group transition-all">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <VaultTypeIcon className="w-5 h-5 text-primary" />
        ...
        <div className="flex gap-2">
          {vault.status === 'ACTIVE' && (
            <>
              <button className="flex-1 px-3 py-2 bg-primary/20 text-primary text-sm font-medium rounded hover:bg-primary/30 transition-colors flex items-center justify-center gap-1.5">
                <Plus className="w-3.5 h-3.5" /> Add
              </button>
              {vault.vaultType === 'FLEXIBLE' && (
                <button className="flex-1 px-3 py-2 bg-accent/20 text-accent text-sm font-medium rounded hover:bg-accent/30 transition-colors flex items-center justify-center gap-1.5">
          ...
          {vault.status === 'MATURE' && (
            <button className="w-full px-3 py-2 bg-green-500/20 text-green-400 text-sm font-medium rounded hover:bg-green-500/30 transition-colors flex items-center justify-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5" /> Claim Now
            </button>
          )}
          {vault.status === 'CLAIMED' && (
            <button className="w-full px-3 py-2 bg-blue-500/20 text-blue-400 text-sm font-medium rounded opacity-60 cursor-default flex items-center justify-center gap-1.5">
